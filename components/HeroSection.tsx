...
const CommunityAvatars = () => {
  ...
  const [phase, setPhase] = useState<'scattered' | 'searching' | 'connecting' | 'community'>('scattered');
  ...
  const avatars = [
    { id: 1, emoji: '👨‍💻', role: 'Developer', gradient: 'from-primary-400 to-primary-600' },
    { id: 2, emoji: '👩‍🎨', role: 'Designer', gradient: 'from-secondary-400 to-secondary-600' },
    { id: 3, emoji: '👨‍💼', role: 'Manager', gradient: 'from-accent-400 to-accent-600' },
    { id: 4, emoji: '👩‍🏫', role: 'Teacher', gradient: 'from-primary-300 to-primary-500' },
    { id: 5, emoji: '👨‍🔬', role: 'Researcher', gradient: 'from-secondary-300 to-secondary-500' },
    { id: 6, emoji: '👩‍💻', role: 'Engineer', gradient: 'from-accent-300 to-accent-500' },
    { id: 7, emoji: '👨‍🎤', role: 'Creative', gradient: 'from-secondary-400 to-primary-400' },
    { id: 8, emoji: '👩‍⚕️', role: 'Consultant', gradient: 'from-primary-400 to-accent-400' },
  ];
  ...
  useEffect(() => {
    ...
    storyTimeline
      .to(avatarRefs.current.filter((_, i) => i % 2 === 0), {
      ...
      .to(avatarRefs.current.filter((_, i) => i % 2 === 1), {
  ...
  return (
    <div ref={containerRef} className="absolute inset-0 pointer-events-none overflow-hidden">
      ...
      <svg className="absolute inset-0 w-full h-full">
        {avatars.map((_, index) => (
          <g key={`connection-${index}`} ref={setConnectionRef(index)} className="opacity-0 scale-0">
            {avatars.slice(index + 1).map((_, j) => {
              ...
              const x1 = windowSize.width / 2 + Math.cos(angle1) * 150;
              const y1 = windowSize.height / 2 + Math.sin(angle1) * 150;
              const x2 = windowSize.width / 2 + Math.cos(angle2) * 150;
              const y2 = windowSize.height / 2 + Math.sin(angle2) * 150;
        ...
        <defs>
          <linearGradient id="gradient" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <stop offset="50%" stopColor="rgb(244, 114, 182)" stopOpacity="0.3" />
      ...
      <div className="community-ring absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
        {avatars.map((avatar, index) => (
          ...
          >
            {/* Avatar bubble */}
            <div className={`relative w-20 h-20 rounded-full bg-gradient-to-br ${avatar.gradient} shadow-lg transform transition-all duration-300`}>
              <div className="avatar-emoji absolute inset-0 flex items-center justify-center text-3xl">
                {avatar.emoji}
              </div>
              
              {/* Role label */}
              <div className={`absolute -bottom-6 left-1/2 -translate-x-1/2 text-xs font-medium text-gray-600 whitespace-nowrap transition-opacity duration-300 ${
                phase === 'community' ? 'opacity-100' : 'opacity-0'
              }`}>
                {avatar.role}
              </div>
...
const HeroSection = () => {
  ...
  useEffect(() => {
    ...
    if (ctaRef.current) {
      gsap.fromTo(
        ...
        { y: 0, opacity: 1, scale: 1, duration: 0.8, stagger: 0.1, delay: 1, ease: 'back.out(1.7)' }
      );
    }
  ...
  return (
    <section className="relative min-h-[90vh] flex items-center justify-center overflow-hidden bg-gradient-to-br from-primary-50 via-white to-secondary-50">
      {/* Background Pattern */}
